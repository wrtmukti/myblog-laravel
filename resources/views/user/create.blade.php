                    ...
                    <div class="flex mt-12 bg-white rounded-md shadow">
                        <div class="flex-grow">
                            <div class="flex flex-col mx-40 mt-20">
                                    ...
                                    <form action="/users/dashboard/ {{ Auth::user()->id }}" method="POST">
                                        @csrf

                                        <input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
                                        <div class="flex items-center mb-4">
                                            <label for="title" class="w-24 font-semibold text-gray-700">Judul</label>
                                            <input type="text" class="flex-grow border border-red-200 rounded py-1 px-3 @error('title') is-invalid @enderror" id="title" name="title" value=" {{ old('title') }}">
                                            @error('title')<div class="invalid-feedback"> {{ $message }}</div>@enderror
                                        </div>
                                        <div class="flex items-center mb-4">
                                            <label for="category" class="w-24 font-semibold text-gray-700">Kategori</label>
                                            <input type="text" class="flex-grow border border-red-200 rounded py-1 px-3 @error('category') is-invalid @enderror" id="category" name="category" value=" {{ old('category') }}">
                                        ...
                                        <div class="flex items-center mb-4">
                                            ...
                                            <textarea type="text" rows="8" class="flex-grow border border-red-200 rounded py-1 px-3  @error('text') is-invalid @enderror" id="text" name="text" value=" {{ old('text') }}"></textarea>
